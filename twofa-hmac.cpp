...
void goog_hmac_sha1(const uint8_t *key, int keyLength,
                    ...
                    uint8_t *result, int resultLength)
{
  ...
  memset(hashed_key, NULL, sizeof(hashed_key));
  memset(sha, NULL, sizeof(sha));
  memset(tmp_key, NULL, sizeof(tmp_key));
}